...
export default function AdminProductListPage() {
  ...
  const selectClass =
    'bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500 text-slate-700 w-full';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">Quản lý Sản phẩm</h1>
          <p className="text-slate-500 mt-1 text-sm">Quản lý danh mục sản phẩm, biến thể và tồn kho.</p>
        </div>
        <Link
          to="/admin/products/new"
          className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-xl font-semibold shadow-sm shadow-indigo-600/20 transition-all active:scale-95 text-sm whitespace-nowrap"
        >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-white text-slate-400 uppercase text-xs font-semibold border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-4 sm:px-6 py-4 hidden sm:table-cell">Phân loại</th>
                  <th className="px-4 sm:px-6 py-4 text-right">Giá bán</th>
                  <th className="px-4 sm:px-6 py-4 text-center hidden md:table-cell">Tồn kho</th>
                  <th className="px-4 sm:px-6 py-4 text-center">Trạng thái</th>
                  <th className="px-4 sm:px-6 py-4 text-right">Hành động</th>
                </tr>
              </thead>
                ...
                {products.map((p) => (
                  ...
                  >
                    ...
                    <td className="px-4 sm:px-6 py-4 text-right">
                      <div className="font-bold text-indigo-600 whitespace-nowrap">{formatCurrency(p.salePrice)}</div>
                      {p.comparePrice && (
                        <div className="text-xs text-slate-400 line-through mt-0.5 whitespace-nowrap">
                    ...
                    <td className="px-4 sm:px-6 py-4 text-center">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${p.status === 1 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
                        }`}>
        ...
        {/* Pagination */}
        <div className="p-4 border-t border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-sm text-slate-500">
          <div>
          ...
          {displayTotalPages > 1 && (
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="ghost" size="sm" disabled={page >= displayTotalPages} onClick={() => setPage((p) => p + 1)} className="rounded-xl border border-slate-200">
                  Tiếp theo
                </Button>
              </div>
            </div>
